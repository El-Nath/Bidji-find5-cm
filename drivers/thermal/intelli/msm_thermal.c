...
static bool started = 1;

static unsigned int debug_mode = 0;
static uint32_t cpus_offlined;
static DEFINE_MUTEX(core_control_enabled_mutex);

/* This is Intelli-thermal don't start on boot */
static int enabled;
static int limit_idx;
static int limit_idx_low = 5; /*594000mhz*/
...
static void __ref do_core_control(long temp)
{
	 ...
	 */
	mutex_lock(&core_control_enabled_mutex);
	...
	mutex_unlock(&core_control_enabled_mutex);
}
#else
...
static ssize_t show_cc_started_dummy(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", enabled);
}

static ssize_t __ref store_cc_started_dummy(struct kobject *kobj,
...
static ssize_t show_cc_started(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", core_control_enabled);
}

static ssize_t __ref store_cc_started(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int val = 0;
...
static __refdata struct kobj_attribute cc_started_attr =
__ATTR(core_control_enabled, 0664, show_cc_started, store_cc_started);

static __refdata struct kobj_attribute cc_started_dummy_attr =
__ATTR(started, 0664, show_cc_started_dummy, store_cc_started_dummy);

static __refdata struct kobj_attribute cpus_offlined_attr =
__ATTR(cpus_offlined, 0664, show_cpus_offlined, store_cpus_offlined);

static __refdata struct attribute *cc_attrs[] = {
	&cc_started_attr.attr,
	&cc_started_dummy_attr.attr,
